  ...
  <section class="pr v-slide-content">
    <ul class="v-slide-box pa" ref="vSlide" :class="{'slide-action': true}">
      <li v-for="item in lists" :key="item.id">
        <img :class="{'click-select': lists[0] === item}"
           :src="`https://gw.alicdn.com/${item.poster}`"
           :data-id="item.id" style="height: 90px"
           alt="">
      </li>
    </ul>
  </section>
  ...
  methods: {
    ...
    computeSlideWidth (slideBox) {
      ...
      Array.prototype.forEach.call(imgEles, (item) => {
        item.onload = function () {
          ...
          self.slideWidth += this.clientWidth
          loadNumber++
          if (loadNumber === this.imgNumber) {
            let winWidth = document.documentElement.clientWidth || document.body.clientWidth
            let slideBoxLength = self.slideWidth + ((self.imgNumber - 1) * self.imgGap)
            self.slideMaxLength = slideBoxLength - winWidth - 8
    ...
    addListenEvent (slideBox) {
      ...
      slideBox.addEventListener('touchstart', (event) => {
        // eslint-disable-next-line
        if (this.clickAction === false) this.slideStart.call(this, event)
      }, false)
      slideBox.addEventListener('touchmove', (event) => {
        // eslint-disable-next-line
        if (this.clickAction === false) this.slideMove.call(this, event, slideBox)
      }, false)
      slideBox.addEventListener('touchend', (event) => {
        // eslint-disable-next-line
        if (this.clickAction === false) this.slideEnd.call(this, event, slideBox)
      }, false)
    }
  }
}
